...
#define AUP_AED_FEA_LEN (AUP_AED_MEL_FILTER_BANK_NUM + 1) // feature length of AIVAD
...
private:
  ...
  float input_data_buf_1234[AUP_AED_MODEL_IO_NUM - 1][AUP_AED_MODEL_HIDDEN_DIM] = {0};
...
typedef struct Aed_St_ {
  ...
  float* frameRmsBuff;         // [frmRmsBufLen], FIFO, to delay frmRms result so that it aligns with AIVAD result
  float* aivadInputFeatStack;  // [...] = [AUP_AED_CONTEXT_WINDOW_LEN * AUP_AED_FEA_LEN]
